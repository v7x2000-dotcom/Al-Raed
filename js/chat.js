...
const ChatManager = {
                ...
                if (member) {
                    ...
                    if (statusEl) {
                        ...
                        const lastSeenText = isOnline ? (document.documentElement.dir === 'rtl' ? 'متصل الآن' : 'Online') : 
                                            (lastSeen ? (document.documentElement.dir === 'rtl' ? 'آخر ظهور ' : 'Last seen ') + App.formatTimeAgo(lastSeen) : (document.documentElement.dir === 'rtl' ? 'غير متصل' : 'Offline'));
    ...
    bindTabEvents: () => {
        document.querySelectorAll('.chat-tab').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                if (msgContainer) msgContainer.innerHTML = '<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;opacity:0.4;gap:1rem;text-align:center;"><i class="fas fa-comments" style="font-size:4rem;"></i><p>اختر محادثة للبدء</p></div>';
            });
        });
    },
    ...
    _selectRoom: (room, e) => {
        ...
        const statusEl = document.getElementById('chat-active-status');
        if (statusEl) statusEl.innerHTML = `<i class="fas fa-users" style="font-size:0.75rem;opacity:0.6;"></i> ${room.members?.length||0} أعضاء`;
        const key = ChatManager._getRoomKey(room.id);
        ChatManager._clearUnread(key);
        ...
        if (input && btn) {
            ...
            if (room.isLocked && !isAdmin) {
                input.placeholder = 'المجموعة مغلقة حالياً - المدير فقط يمكنه الكتابة';
                input.disabled = true;
                btn.disabled = true;
    ...
    renderMessages: () => {
        ...
        container.innerHTML = '';
        if (!messages.length) {
            container.innerHTML = '<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;opacity:0.6;gap:1rem;text-align:center;padding:2rem;"><i class="fas fa-comment-dots" style="font-size:3rem;color:var(--primary-color);"></i><p style="font-size:1rem;font-weight:600;">لا توجد رسائل بعد. قل مرحباً! 👋</p></div>';
            return;
        }
    ...
    toggleProfileSidebar: () => {
        ...
        if (isPrivate) {
            ...
            name = room.name; subtitle = ChatManager.currentType==='group'?'مجموعة عمل':'قناة';
            ...
            if (canEdit || isActualAdmin) {
                adminTools = `
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:1rem;">
                        <button onclick="ChatManager.showRoomEditModal('${room.id}')" style="background:rgba(37,99,235,0.1);color:var(--primary-color);border:1px solid rgba(37,99,235,0.2);padding:8px;border-radius:10px;font-size:0.8rem;font-weight:700;cursor:pointer;"><i class="fas fa-edit"></i> تعديل</button>
                        ...
                        ${isActualAdmin ? `<button onclick="ChatManager.showPrivacySettingsModal('${room.id}')" style="background:rgba(245,158,11,0.1);color:#f59e0b;border:1px solid rgba(245,158,11,0.2);padding:8px;border-radius:10px;font-size:0.8rem;font-weight:700;cursor:pointer;grid-column: span 2;"><i class="fas fa-shield-alt"></i> إعدادات الخصوصية</button>` : ''}
                    </div>`;
            }
        ...
        modal.innerHTML = `
            <div class="modal-content glass-effect" style="max-width:450px;">
                ...
                <div class="modal-footer">
                    ...
                    <button class="btn btn-primary" id="btn-save-room">حفظ التغييرات</button>
                </div>
            </div>
        `;
    ...
    showMemberActionsModal: (roomId, memberId) => {
        ...
        modal.innerHTML = `
            <div class="modal-content glass-effect" style="max-width:320px;text-align:center;padding:2rem;background:var(--bg-secondary);border:1px solid var(--primary-color);">
                ...
                <p style="font-size:0.8rem;opacity:0.6;margin-bottom:1.5rem;color:var(--text-secondary);">إدارة صلاحيات العضو في هذه المجموعة</p>
                
                <div style="display:flex;flex-direction:column;gap:10px;">
                    ${isTargetAdmin ? 
                        `<button class="btn btn-secondary" onclick="ChatManager.demoteFromAdmin('${roomId}', '${memberId}', this.closest('.modal'))" style="width:100%;justify-content:center;"><i class="fas fa-user-minus"></i> إزالة من الإشراف</button>` :
                        `<button class="btn btn-primary" onclick="ChatManager.promoteToAdmin('${roomId}', '${memberId}', this.closest('.modal'))" style="width:100%;justify-content:center;"><i class="fas fa-user-shield"></i> تعيين كمشرف</button>`
                    }
                    <button class="btn btn-outline" onclick="ChatManager.removeMember('${roomId}', '${memberId}', this.closest('.modal'))" style="width:100%;color:#f43f5e;border-color:#f43f5e;justify-content:center;"><i class="fas fa-user-times"></i> حذف من المجموعة</button>
                    <button class="btn btn-secondary" onclick="this.closest('.modal').remove()" style="width:100%;background:rgba(255,255,255,0.05);color:#fff;justify-content:center;">إغلاق</button>
                </div>
            </div>
        `;
        document.body.appendChild(modal);
    ...
    promoteToAdmin: (roomId, memberId, modalElement) => {
        ...
        if (room) {
            ...
            if (!room.admins.includes(memberId)) {
                ...
                Store.set('chat_rooms', rooms);
                NotificationManager.add('تم التعيين كمشرف بنجاح', 'fa-user-shield', 'success');
    ...
    demoteFromAdmin: (roomId, memberId, modalElement) => {
        ...
        if (room && room.admins) {
            if (memberId === room.createdBy) {
                NotificationManager.add('لا يمكن إزالة المنشئ من الإشراف', 'fa-exclamation-triangle', 'warning');
                return;
            }
    ...
    removeMember: (roomId, memberId, modalElement) => {
        askConfirm('هل أنت متأكد من حذف هذا العضو من المجموعة؟', () => {
            ...
            if (room) {
                ...
                Store.set('chat_rooms', rooms);
                NotificationManager.add('تم حذف العضو من المجموعة', 'fa-user-times', 'success');
    ...
    showAddMembersModal: (roomId) => {
        ...
        modal.innerHTML = `
            <div class="modal-content glass-effect" style="max-width:450px;">
                <div class="modal-header">
                    <h2>إضافة أعضاء جدد</h2>
                    <button class="close-modal"><i class="fas fa-times"></i></button>
                </div>
                <div class="modal-body">
                    <div style="max-height:300px;overflow-y:auto;padding-right:5px;">
                        ${nonMembers.length === 0 ? '<p style="text-align:center;opacity:0.6;">جميع أعضاء الفريق موجودون بالفعل</p>' : nonMembers.map(u => `
                ...
                <div class="modal-footer">
                    <button class="btn btn-secondary cancel-modal">إلغاء</button>
                    <button class="btn btn-primary" id="btn-confirm-add" ${nonMembers.length === 0 ? 'disabled' : ''}>إضافة الأعضاء المحددين</button>
                </div>
            </div>
        `;
        document.body.appendChild(modal);
        ...
        modal.querySelector('#btn-confirm-add').onclick = () => {
            ...
            if (r) {
                ...
                NotificationManager.add(`تمت إضافة ${selectedIds.length} أعضاء بنجاح`, 'fa-user-plus', 'success');
                modal.remove();
                ChatManager.toggleProfileSidebar();
            }
        };
        ...
        const send = () => {
            ...
            if (ChatManager.currentType !== 'private') {
                ...
                if (room && room.isLocked && !isAdmin) {
                    NotificationManager.add('المجموعة مغلقة حالياً، المدير فقط يمكنه الكتابة', 'fa-lock', 'warning');
                    return;
                }
            }
    ...
    deleteConversation: () => {
        ...
        if (modal.querySelector('#btn-del-all')) {
            modal.querySelector('#btn-del-all').onclick = () => {
                askConfirm('هل أنت متأكد من حذف هذه المحادثة لدى جميع الأطراف؟ لا يمكن التراجع عن هذا الإجراء.', () => {
                    const convKey = isPrivate
                        ? (ChatManager._isSelfChat ? ChatManager._getSelfKey(me) : ChatManager._getPrivateKey(me, ChatManager.currentReceiverId))
                        : ChatManager._getRoomKey(ChatManager.currentReceiverId);
    ...
    editMessage: (msgId) => {
        ...
        askPrompt('تعديل الرسالة:', msg.content.replace(' (معدلة)', ''), (newText) => {
    ...
    showCreateRoomModal: (type) => {
        ...
        modal.innerHTML = `
            <div class="modal-content glass-effect" style="max-width:500px; padding:0; border-radius:20px; overflow:hidden;">
                <div style="padding:1.5rem; background:linear-gradient(135deg, var(--primary-color), var(--accent-color)); color:#fff; display:flex; justify-content:space-between; align-items:center;">
                    <h2 style="margin:0; font-size:1.2rem;"><i class="fas ${isGroup?'fa-users':'fa-bullhorn'}"></i> ${isGroup?'إنشاء مجموعة جديدة':'إنشاء قناة جديدة'}</h2>
                ...
                <div class="modal-body" style="padding:1.5rem; display:flex; flex-direction:column; gap:1.2rem;">
                    ...
                    <div>
                        <label style="display:block; font-size:0.8rem; font-weight:700; margin-bottom:6px; opacity:0.8;">اسم ${isGroup?'المجموعة':'القناة'}</label>
                        <input type="text" id="new-room-name" placeholder="مثلاً: فريق التطوير" style="width:100%; padding:0.8rem; border-radius:12px; background:var(--bg-primary); border:1.5px solid var(--border-color); color:var(--text-primary); outline:none; transition:border 0.3s;">
                    </div>
                    <div>
                        <label style="display:block; font-size:0.8rem; font-weight:700; margin-bottom:6px; opacity:0.8;">الوصف</label>
                        <textarea id="new-room-desc" placeholder="توضيح موجز للهدف..." style="width:100%; padding:0.8rem; border-radius:12px; background:var(--bg-primary); border:1.5px solid var(--border-color); color:var(--text-primary); height:80px; outline:none; resize:none;"></textarea>
                    </div>
                    <div>
                        <label style="display:block; font-size:0.8rem; font-weight:700; margin-bottom:8px; opacity:0.8;">دعوة الأعضاء (الخصوصية)</label>
        ...
        modal.querySelector('#btn-confirm-room').onclick = () => {
            ...
            NotificationManager.add('تم إنشاء ' + (isGroup?'المجموعة':'القناة') + ' بنجاح', 'fa-check-circle', 'success');
            modal.remove();
            ChatManager.loadRooms();
        };
